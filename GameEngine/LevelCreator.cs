        ...
        private static void MinimizeHallways(Level level)
        {
            ...
            foreach (var hallway in level.GetHallways())
            {
                ...
                foreach (var feature in hallway.FeatureList)
                {
                    ...
                    {
                        feature.AssignedIntersectional = Intersectional.BottomTopLeft;
                    }
                    else if (!upIsNull && !downIsNull && leftIsNull && !rightIsNull)
                    {
                        feature.AssignedIntersectional = Intersectional.BottomTopRight;
                    }
                    else if (upIsNull && downIsNull && !leftIsNull && !rightIsNull)
                    {
                        feature.AssignedIntersectional = Intersectional.LeftRight;
                    }
                    else if (upIsNull && downIsNull && leftIsNull && rightIsNull)
                    {
                        feature.AssignedIntersectional = Intersectional.BottomTopLeftRight;
                    }
                    else if (!upIsNull && !downIsNull && leftIsNull && rightIsNull)
                    {
                        feature.AssignedIntersectional = Intersectional.BottomTop;
                    }
                    else if (upIsNull && !downIsNull && !leftIsNull && rightIsNull)
                    {
                        feature.AssignedIntersectional = Intersectional.TopRight;
                    }
                    else if (!upIsNull && downIsNull && !leftIsNull && rightIsNull)
                    {
                        feature.AssignedIntersectional = Intersectional.BottomRight;
                    }
                    else if (!upIsNull && downIsNull && leftIsNull && !rightIsNull)
                    {
                        feature.AssignedIntersectional = Intersectional.BottomLeft;
                    }
                    else if (upIsNull && !downIsNull && leftIsNull && !rightIsNull)
                    {
                        feature.AssignedIntersectional = Intersectional.TopLeft;
                    }
                    else if (!upIsNull && downIsNull && !leftIsNull && !rightIsNull)
                    {
                        feature.AssignedIntersectional = Intersectional.BottomLeftRight;
                    }
                    else if (upIsNull && !downIsNull && !leftIsNull && !rightIsNull)
                    {
                        feature.AssignedIntersectional = Intersectional.TopLeftRight;
                    }
